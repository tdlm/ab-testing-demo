...
import {HERO_TEXT_VARIANT_BY_ID_QUERY, HERO_TEXT_VARIANT_QUERY} from '@/sanity/lib/queries'
...
export const getHeroTextVariant = async () => {
    ...
    const totalWeight = heroTextVariants.reduce((sum: number, variant: { distributionWeight: number }) => 
      sum + variant.distributionWeight, 0);
    ...
    let weightSum = 0;
    const selectedHeroTextVariant = heroTextVariants.find((variant: { distributionWeight: number }) => {
    ...
    const heroTextVariant = await client.fetch(HERO_TEXT_VARIANT_BY_ID_QUERY, {variantId: selectedHeroTextVariant.variantId})
