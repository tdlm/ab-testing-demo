export const heroTextVariant = {
    ...
    fields: [
      {
        ...
        validation: (Rule: any) => Rule.required().warning('A variant ID is recommended')
      },
      {
        name: 'heroTitle',
        title: 'Hero Title',
      ...
      },
      {
        ...
        validation: (Rule: any) => Rule
          ...
          .max(100)
          .integer()
          .error('Distribution weight must be a whole number between 0 and 100')
      },
      {
        name: 'active',
        title: 'Active',
    ...
    preview: {
      ...
      prepare({ title = 'Untitled', subtitle ='', weight = 0, active = false }: any) {
        return {
          title: `${title} (${weight}%)`,
